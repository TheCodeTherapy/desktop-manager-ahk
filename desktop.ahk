; Marco Gomez ( @marcogomez_ - https://mgz.me )
; Setup
#NoEnv
...
; Timer run until process is killed
...
; ...CurrentVersion\Explorer\SessionInfo\1\VirtualDesktops
...
mapDesktopsFromRegistry() {
	...
	; but there's no guarantee this couldn't change in a later
	; Windows release so we check.
	IdLength := 32
	...
	while (CurrentDesktopId and i < DesktopCount) {
		...
		; If we didn't find anything, keep the
...
; Switches to the desktop which number to was provided.
switchDesktopByNumber(targetDesktop)
{
	global CurrentDesktop, DesktopCount
	; Re-generate the list of desktops and where we fit in that.
	; We do this because the user may have switched desktops
	; via some other means than the script.
	mapDesktopsFromRegistry()
